...
void C3PO::newPosition(map& map) 
{
	...
	{
			p = { this->getCurrentPosition().first + 1, this->getCurrentPosition().second }; //posibila noua pozitie: jos
			...
			{
				if (getCurrentPosition().first % 2 == 1)
					p = { getCurrentPosition().first + 1, getCurrentPosition().second - 1 };
				else
				{
					bool ok = false;
					if (getCurrentPosition().first != 0 && ok == false)
					{
	...
	if (this->getCurrentPosition().second == 0 && this->getCurrentPosition().first != 0)
	{
		...
		if (map.getMatrix()[p.first][p.second] == 'T' && getItems() == 0)  //daca nu mai are niciun item pe care sa il foloseasca si pozitia are o capcana
		{
			if (getCurrentPosition().first % 2 == 0)
				p = { getCurrentPosition().first + 1, getCurrentPosition().second + 1 };
			else
			{
				bool ok = false;
	...
	{
		p = { this->getCurrentPosition().first, this->getCurrentPosition().second + 1 }; //posibila noua pozitie: dreapta
	...
	if (getCurrentPosition().first % 2)
	{
		p = { this->getCurrentPosition().first, this->getCurrentPosition().second - 1 }; //posibila noua pozitie: stanga
